<template>
  <div>
    <div class="shopcart">
      <div class="content">
        <div class="content-left" @click="toggleShow">
          <div class="logo-wrapper">
            <div class="logo" :class="{ highlight: totalCount }">
              <svg class="icon" aria-hidden="true">
                <use xlink:href="#icon-gouwuche1"></use>
              </svg>
            </div>
            <div class="num" v-if="totalCount">{{ totalCount }}</div>
          </div>
          <div class="price">￥{{ totalPrice }}</div>
          <div class="desc">另需配送费￥{{ info.deliveryPrice }} 元</div>
        </div>
        <div class="content-right">
          <div class="pay" :class="payClass">{{ payText }}</div>
        </div>
      </div>
      <transition name="move">
        <div class="shopcart-list" v-show="showList">
          <div class="list-header">
            <h1 class="title">购物车</h1>
            <span class="empty" @click="clearCart">清空</span>
          </div>
          <div class="list-content">
            <ul>
              <li class="food" v-for="(foods, index) in cartFoods" :key="index">
                <span class="name">{{ foods.name }}</span>
                <div class="price">
                  <span>￥{{ foods.price }}</span>
                </div>
                <div class="cartcontrol-wrapper">
                  <div class="cartcontrol">
                    <CartControl :foods="foods" />
                  </div>
                </div>
              </li>
            </ul>
          </div>
        </div>
      </transition>
    </div>
    <div class="list-mask" v-show="showList" @click="toggleShow"></div>
  </div>
</template>

<script>
import BetterScroll from "better-scroll";
import CartControl from "../CartControl/CartControl.vue";
import { computed, ref ,nextTick, reactive} from "vue";
import { useStore } from "vuex";
export default {
  components: { CartControl },
  setup() {
    const store = useStore();
    const cartFoods = computed(() => store.state.cartFoods);
    const info = computed(() => store.state.info);
    const totalCount = computed(() => store.getters.totalCount);
    const totalPrice = computed(() => store.getters.totalPrice);
    const isShow = ref(false);
	
    const toggleShow = () => {
      isShow.value = !isShow.value;
    };
    const showList = computed(() => {
		
      if (totalCount.value === 0) {
        isShow.value = false;
        return false;
      }
	  if(isShow.value){
		  let scroll
		  if(!scroll){
			  console.log(scroll);
			 scroll =nextTick(() => {
				   new BetterScroll(".list-content",{
					   click:true
				   });
			  })
		  }else{
			  scroll.refresh()
		  }	
	  }
      return isShow.value;
    });

    const payText = computed(() => {
      if (totalPrice.value === 0) {
        return `￥${info.value.minPrice}元起送`;
      } else if (totalPrice.value < info.value.minPrice) {
        return `还差${info.value.minPrice - totalPrice.value}元`;
      } else {
        return "结算";
      }
    });
    const payClass = computed(() => {
      console.log(info.value.minPrice);
      return totalPrice.value >= info.value.minPrice ? " enough" : "not-enough";
    });
    const clearCart = () => {
		store.dispatch('clearCart')
    };

    return {
      cartFoods,
      info,
      totalCount,
      totalPrice,
      payText,
      payClass,
      isShow,
      toggleShow,
      showList,
      clearCart,
    };
  },
};
</script>
<style lang="less" scoped>
.shopcart {
  position: fixed;
  left: 0;
  bottom: 0;
  z-index: 50;
  width: 100%;
  height: 48px;
  .content {
    display: flex;
    background: #141d27;
    font-size: 0;
    color: rgba(255, 255, 255, 0.4);
    .content-left {
      flex: 1;
      .logo-wrapper {
        display: inline-block;
        vertical-align: top;
        position: relative;
        top: -10px;
        margin: 0 12px;
        padding: 6px;
        width: 56px;
        height: 56px;
        box-sizing: border-box;
        border-radius: 50%;
        background: #141d27;
        .logo {
          width: 100%;
          height: 100%;
          border-radius: 50%;
          text-align: center;
          &.highlight {
            background: green;
            // .icon-shopping_cart {
            //   line-height: 44px;
            //   font-size: 24px;
            //   color: #f5f5f5;
            //   &.highlight {
            //     color: #fff;
            //   }
            // }
          }
          .icon {
            width: 45px;
            height: 45px;
            border-radius: 50%;
          }
        }
        .num {
          position: absolute;
          top: 0;
          right: 0;
          width: 24px;
          height: 16px;
          line-height: 16px;
          text-align: center;
          border-radius: 16px;
          font-size: 9px;
          font-weight: 700;
          color: #ffffff;
          background: rgb(240, 20, 20);
          box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.4);
        }

        .desc {
          display: flex;
          color: #fff;
        }
      }
      .price {
        display: inline-block;
        vertical-align: top;
        margin-top: 2px;
        line-height: 24px;
        padding-right: 12px;
        box-sizing: border-box;
        font-size: 15px;
        font-weight: 700;
        color: #fff;
        &.highlight {
          color: #fff;
        }
      }
      .desc {
        display: inline-block;
        vertical-align: bottom;
        margin-bottom: 15px;
        margin-left: -45px;
        font-size: 10px;
      }
    }
    .content-right {
      flex: 0 0 105px;
      width: 105px;
      .pay {
        height: 48px;
        line-height: 48px;
        text-align: center;
        font-size: 12px;
        font-weight: 700;
        color: #fff;
        &.not-enough {
          background: #2b333b;
        }
        &.enough {
          background: #00b43c;
          color: #fff;
        }
      }
    }
  }
  .list-content {
    padding: 0 18px;
    max-height: 8rem;
    overflow: hidden;
    background: #fff;
    .food {
      position: relative;
      padding: 12px 0;
      box-sizing: border-box;
      border: 1px rgba(7, 17, 27, 0.1);
      .name {
        line-height: 24px;
        font-size: 14px;
        color: rgb(7, 17, 27);
      }
      .price {
        position: absolute;
        right: 90px;
        bottom: 12px;
        line-height: 24px;
        font-size: 14px;
        font-weight: 700;
        color: rgb(240, 20, 20);
      }
      .cartcontrol-wrapper {
        position: absolute;
        right: 0;
        bottom: 13px;
      }
    }
  }

  .ball-container {
    .ball {
      position: fixed;
      left: 32px;
      bottom: 22px;
      z-index: 200;
      transition: all 0.4s cubic-bezier(0.49, -0.29, 0.75, 0.41);
      .inner {
        width: 16px;
        height: 16px;
        border-radius: 50%;
        background: green;
        transition: all 0.4s linear;
      }
    }
  }
  .shopcart-list {
    position: absolute;
    left: 0;
    top: 0;
    z-index: -1;
    width: 100%;
    transform: translateY(-100%);
    &.move-enter-from,
    &.move-leave-to {
		transform: translateY(0)
    };
    &.move-leave-from,
    &.move-enter-to {
		transform: translateY(-100%)
    };
    &.move-enter-active,
    &.move-leave-active {
      transition: all .5s ;
    };
    .list-header {
      height: 40px;
      padding: 0 18px;
      background: #f3f5f7;
      border-bottom: 1px solid rgba(7, 17, 27, 0.1);
      .title {
        float: left;
        font-size: 14px;
        color: rgb(7, 17, 27);
      }
      .empty {
        margin-top: 13px;
        float: right;
        font-size: 12px;
        color: rgb(0, 160, 220);
      }
    }
  }
  .highlight {
    background: green;
  }
}
.list-mask {
  position: fixed;
  top: 0;
  left: 0;
  width: 15rem;
  height: 26.68rem;
  z-index: 40;
  backdrop-filter: blur(10px);
  opacity: 1;
  background: rgba(7, 17, 27, 0.6);
  &.fade-enter-active,
  &.fade-leave-active {
    transition: all .8s ;
  }
  &.fade-enter,
  &.fade-leave-to {
    opacity: 0;
    background: rgba(7, 17, 27, 0);
  }
}
</style>
